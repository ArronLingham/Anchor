...
private let contrastControlTargets: Set<String> = [
    "contrast",
    "hardwareContrast",
]
...
final class BetterDisplayManager: ObservableObject {
    ...
    private func routeOSDToHUD(_ osd: BetterDisplayOSDNotification) {
        ...
        case .contrast:
            dispatchContrastHUD(value: normalizedValue, customSymbol: "circle.lefthalf.filled", onScreen: targetScreen)
        ...
        case .other:
            // For unsupported control targets (gamma, temperature, etc.),
    ...
    private func dispatchContrastHUD(value: CGFloat, customSymbol: String? = nil, onScreen targetScreen: NSScreen? = nil) {
        let icon = customSymbol ?? "circle.lefthalf.filled"
        if Defaults[.enableCircularHUD] {
            CircularHUDWindowManager.shared.show(type: .contrast, value: value, icon: icon, onScreen: targetScreen)
            return
        }
        if Defaults[.enableVerticalHUD] {
            VerticalHUDWindowManager.shared.show(type: .contrast, value: value, icon: icon, onScreen: targetScreen)
            return
        }
        if Defaults[.enableCustomOSD] && Defaults[.enableOSDBrightness] {
            CustomOSDWindowManager.shared.show(type: .contrast, value: value, icon: icon, onScreen: targetScreen)
        }
        if Defaults[.enableSystemHUD] && !Defaults[.enableCustomOSD] && !Defaults[.enableVerticalHUD] && !Defaults[.enableCircularHUD] {
            coordinator?.toggleSneakPeek(
                status: true,
    ...
    private func classifyControlTarget(_ target: String?, systemIconID: Int?) -> BetterDisplayControlCategory {
        if let target {
            ...
            if contrastControlTargets.contains(target) { return .contrast }
    ...
    /// Set contrast for a specific displayID via BetterDisplay integration.
    func setContrast(_ value: Double, forDisplayID displayID: Int) {
        let percentage = max(0, min(100, value * 100))
        sendRequest(
