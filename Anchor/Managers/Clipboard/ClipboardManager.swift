...
class ClipboardManager: ObservableObject {
    ...
    private func checkClipboard() {
        ...
        // Clean tracking parameters before the item is filed, so history stores
        // the cleaned link rather than the original. Rewriting the pasteboard
        // bumps the change count, and the rewrite registers that new count as
        // self-generated — so this poll files the cleaned value and the next
        // poll ignores the write.
        // `checkClipboard` is driven by `scheduleClipboardTimer`'s
        // `Timer.scheduledTimer`, which schedules on the current run loop —
        // the main one. `assumeIsolated` states that rather than assuming it
        // silently, and traps at this line if it ever stops holding.
        MainActor.assumeIsolated {
            let tools = ClipboardToolsManager.shared
            if let raw = NSPasteboard.general.string(forType: .string) {
                // Rewriting bumps the change count and registers the new one as
                // self-generated, so this poll files the cleaned value and the
                // next poll ignores the write.
                _ = tools.cleanURLIfNeeded(raw)
            }
            // Arms the auto-clear countdown. No-op unless that feature is on.
            tools.noteClipboardChanged()
        }
