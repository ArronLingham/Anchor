...
final class VerticalHUDWindowManager {
    ...
    private func ensureWindow(for type: SneakContentType, screen: NSScreen) -> OSDWindow {
        ...
        let state = VerticalHUDState(type: type, value: 0, icon: "", screen: screen)
    ...
    private func updateContent(window: OSDWindow, type: SneakContentType, value: CGFloat, icon: String) {
        ...
        if type == .brightness {
            window.state.contrastValue = CGFloat(SystemBrightnessController.shared.currentContrast(for: window.state.screen) ?? -1) >= 0 ? CGFloat(SystemBrightnessController.shared.currentContrast(for: window.state.screen)!) : nil
        } else {
            window.state.contrastValue = nil
        }
    }
