...
final class SystemStatsManager: ObservableObject {
    ...
    struct Sample: Equatable {
        ...
        /// Compressed/swapped-out memory. A machine with healthy pressure but
        /// heavy swap is a different problem from one that is simply full,
        /// which is why this is reported separately rather than folded in.
        var swapUsedBytes: UInt64 = 0
        /// Bytes moved since monitoring started, so a session total can be
        /// shown alongside the instantaneous rate.
        var networkInTotalBytes: UInt64 = 0
        var networkOutTotalBytes: UInt64 = 0
    ...
    private func sample() {
        ...
        if let net = readNetworkRates() {
            ...
            next.networkInTotalBytes = sessionInBytes
            next.networkOutTotalBytes = sessionOutBytes
        }
        if let swap = readSwapUsedBytes() { next.swapUsedBytes = swap }
    ...
    /// Swap in use, from the kernel's own accounting rather than by inference.
    ...
    /// Running totals for the session. Reset with the counters themselves when
    /// monitoring stops, since a delta across a gap is meaningless.
    private var sessionInBytes: UInt64 = 0
    private var sessionOutBytes: UInt64 = 0
