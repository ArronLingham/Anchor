...
final class SystemBrightnessController {
    ...
    func setBrightnessImmediate(_ value: Float) {
        setBrightnessImmediate(value, for: nil)
    }

    func setBrightnessImmediate(_ value: Float, for screen: NSScreen?) {
        let clamped = max(0, min(1, value))
        markUserInitiated()
        
        // Cancel any running animation so it doesn't fight us
        brightnessAnimationTimer?.invalidate()
        brightnessAnimationTimer = nil
        
        // Update baseline so next animation starts from here
        lastEmittedBrightness = clamped
        
        // Apply immediately for the specified screen (or default)
        applyBrightness(clamped, targetScreen: screen)
        
        // Emit change (throttled to avoid notification storms)
        emitBrightnessChange(value: clamped)
    }

    func setContrastImmediate(_ value: Float, for screen: NSScreen? = nil) {
        let clamped = max(0, min(1, value))
        DispatchQueue.main.async { [weak self] in
            self?.applyContrast(clamped, targetScreen: screen)
        }
    }

    @MainActor
    private func applyContrast(_ value: Float, targetScreen: NSScreen? = nil) {
        let clamped = max(0, min(1, value))
        let targetDisplayID: CGDirectDisplayID
        if let screen = targetScreen, let screenDisplayID = screen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? CGDirectDisplayID {
            targetDisplayID = screenDisplayID
        } else {
            targetDisplayID = displayID
        }

        if setContrastViaDisplayServices(clamped, displayID: targetDisplayID) {
            return
        }

        if BetterDisplayManager.shared.isRunning {
            BetterDisplayManager.shared.setContrast(Double(clamped), forDisplayID: Int(targetDisplayID))
        }
    }

    private func setContrastViaDisplayServices(_ value: Float, displayID explicitDisplayID: CGDirectDisplayID? = nil) -> Bool {
        let targetID = explicitDisplayID ?? displayID
        guard let status = DisplayServicesDynamic.shared.setContrast(displayID: targetID, value: value) else {
            return false
        }
        return status == kIOReturnSuccess
    }

    func currentContrast(for targetScreen: NSScreen? = nil) -> Float? {
        let targetDisplayID: CGDirectDisplayID
        if let screen = targetScreen, let screenDisplayID = screen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? CGDirectDisplayID {
            targetDisplayID = screenDisplayID
        } else {
        ...
        // Avoid querying built-in displays (they typically lack hardware contrast via DDC)
        if CGDisplayIsBuiltin(targetDisplayID) != 0 {
            return nil
        }

        if let res = DisplayServicesDynamic.shared.getContrast(displayID: targetDisplayID), res.status == kIOReturnSuccess {
            return res.value
        }
        return nil
    }
    ...
    private func applyBrightness(_ value: Float, targetScreen: NSScreen? = nil) {
        let clamped = max(0, min(1, value))
        let targetDisplayID: CGDirectDisplayID
        if let screen = targetScreen, let screenDisplayID = screen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? CGDirectDisplayID {
            targetDisplayID = screenDisplayID
        ...
        if targetScreen == nil || CGDisplayIsBuiltin(targetDisplayID) != 0 {
            if coreBrightnessClient.setBrightness(clamped) {
                return
            }
        }
        if setBrightnessViaDisplayServices(clamped, displayID: targetDisplayID) {
            return
        }
    ...
    private func setBrightnessViaDisplayServices(_ value: Float, displayID explicitDisplayID: CGDirectDisplayID? = nil) -> Bool {
        let targetID = explicitDisplayID ?? displayID
        guard let status = DisplayServicesDynamic.shared.setBrightness(displayID: targetID, value: value) else {
            return false
        }
        if status == kIOReturnSuccess {
            return true
        ...
        let retryID = explicitDisplayID ?? displayID
        guard let retry = DisplayServicesDynamic.shared.setBrightness(displayID: retryID, value: value) else {
